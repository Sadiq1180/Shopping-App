...
import 'package:shopping_app/shared/console.dart';
...
class MockInterceptor extends Interceptor {
  ...
  MockInterceptor() {
    _futureManifestLoaded = rootBundle.loadString('AssetManifest.json').then((
      manifestContent,
    ) {
  ...
  void onRequest(
    ...
    handler.resolve(
      Response(
        data: json.decode(
          resData,
        ), // i decoded it to make same as real data it was encoded previously
        requestOptions: options,
        statusCode: statusCode,
      ),
    );
  ...
  String _templateData(
    ...
    var exTemplate = Template(syntax: [_exSyntax], value: sContent);
